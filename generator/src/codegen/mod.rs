...
fn generate_property_declaration(
    ...
    p.newline()?;
    generate_serialize("crate::codec::SerializeValue", name, &spec.properties, p)?;
...
fn generate_resource_declaration(
    ...
    p.newline()?;
    p.block(format_args!("impl crate::Resource for {}", name), |p| {
    ...
    if let Some(ref type_name) = spec.type_ {
        if type_name == "List" {
            if let Some(ref item_type) = spec.item_type {
                format!(
                    "crate::ValueList<{}>",
                    generate_type(namespace_opt, item_type)
                )
            } else {
                format!(
                    "crate::ValueList<{}>",
            ...
            format!("crate::Value<{}>", generate_type(namespace_opt, type_name))
        }
    } else {
        format!(
            "crate::Value<{}>",
...
fn generate_primitive_type(primitive_type: &PrimitiveType) -> &str {
    match primitive_type {
        ...
        &PrimitiveType::Json => "crate::json::Value",
    }
}
